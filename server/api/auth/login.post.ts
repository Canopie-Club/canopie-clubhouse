...
import { SiteExtraType, SiteSummary } from '~/assets/types/db'
...
export default defineEventHandler(async (event) => {
    ...
    const sites = await useDrizzle()
        ...
        .from(tables.siteUsers).where(eq(tables.siteUsers.userId, user.id))
        .innerJoin(tables.sites, eq(tables.sites.id, tables.siteUsers.siteId))

    for (const site of sites) {
        const extras: SiteExtraType[] = (await useDrizzle()
