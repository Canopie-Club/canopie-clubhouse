...
function absoluteRect(node: Element) {
  ...
  const parent = node.closest('.ProseMirror')
  const parentData = parent?.getBoundingClientRect()
  ...
  return {
    top: data.top - (parentData?.top || 0),
    left: data.left - (parentData?.left || 0),
    width: data.width,
    height: data.height,
  }
}
    ...
    props: {
      handleDOMEvents: {
        mousemove: (view, event) => {
          ...
          dragHandleElement.style.top = `${rect.top + (rect.height - dragHandleHeight) / 2}px`
          showDragHandle()
        },
        keydown: () => {
          hideDragHandle()
